...
export default function SelectableTable({
  ...
  const renderTable = () => (
    <div className="overflow-auto">
      <table className="w-full text-sm rtl:text-right ltr:text-left text-neutral-800">
        <thead className="text-neutral-700 uppercase border-b-2 border-neutral-500 bg-neutral-100">
          <tr>
            {showSelection && (
                  ...
                  {sortOptions.find(opt => opt.field === column.field) && (
                    ...
                    >
                      {sortField === column.field ? (
                        <i className={`fa-solid fa-sort-${sortDirection === 'asc' ? 'up' : 'down'} text-purple-500`}></i>
                      ) : (
        ...
        <tbody>
          {data.length > 0 ? (
            data.map((item, index) => {
              ...
              const defaultRowClass = index % 2 === 0 ? 'bg-neutral-100' : 'bg-neutral-50';

              return (
                <tr
                  key={item[idField]}
                  className={`transition-colors ${customRowClass || defaultRowClass} border-b border-neutral-300 ${isSelected ? 'bg-purple-50' : ''} ${onRowClick ? 'cursor-pointer' : ''}`}
